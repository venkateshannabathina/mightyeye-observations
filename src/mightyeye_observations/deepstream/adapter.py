...
class DeepStreamAdapter:
    def __init__(
        ...
        *,
        ...
        self.zone_priority = zone_priority
    ...
    def convert_object(
        ...
        *,
        ...
        if (
            not all(math.isfinite(v) for v in (left, top, width, height))
            or width <= 0
            or height <= 0
        ):
        ...
        box = (
            clip(left / frame_width),
            clip(top / frame_height),
            clip((left + width) / frame_width),
            clip((top + height) / frame_height),
        )
        score = float(obj_meta.confidence)
        # NVIDIA uses -0.1 for unavailable detector confidence.
        confidence = (
            None if score == -0.1 or math.isclose(score, -0.1, abs_tol=1e-7) else score
        )
        ...
        track = (
            None
            if raw_track == UNTRACKED_OBJECT_ID
            else f"{self.session_id}:{raw_track}"
        )
        identity = json.dumps(
            [
                self.camera_id,
        ...
        return Observation(
            ...
            entity_type=self.class_map.get(int(obj_meta.class_id), "unknown"),
            bbox=box,
    ...
    def convert_frame(
        self, frame_meta, *, timestamp: datetime, frame_width: int, frame_height: int
    ) -> list[Observation]:
    ...
    def analytics(self, obj_meta, bindings):
        """Extract nvdsanalytics object metadata inside the vendor boundary."""
        node = getattr(obj_meta, "obj_user_meta_list", None)
        zones, lines, directions = set(), set(), set()
        available = False
        while node is not None:
            user = bindings.NvDsUserMeta.cast(node.data)
            if user.base_meta.meta_type == bindings.nvds_get_user_meta_type(
                "NVIDIA.DSANALYTICSOBJ.USER_META"
            ):
                available = True
                info = bindings.NvDsAnalyticsObjInfo.cast(user.user_meta_data)
                zones.update(str(x) for x in info.roiStatus)
                lines.update(str(x) for x in info.lcStatus)
                if info.dirStatus:
                    directions.add(str(info.dirStatus))
            try:
                node = node.next
            except StopIteration:
                break
        primary = next((z for z in self.zone_priority if z in zones), None)
        if primary is None and zones:
            primary = sorted(zones)[0]
        # Conflicting direction labels remain unknown rather than choosing arbitrarily.
        return {
            "zone": primary,
            "direction": next(iter(directions)) if len(directions) == 1 else None,
            "line_crossing": tuple(sorted(lines)) if available else None,
        }
