...
from mightyeye_observations.deepstream.adapter import (
    UNTRACKED_OBJECT_ID,
    DeepStreamAdapter,
)
...
from mightyeye_observations.world_state import WorldState, replay


def convert(**overrides):
    obj = NS(
        rect_params=NS(left=100, top=100, width=200, height=300),
        confidence=0.9,
        object_id=7,
        class_id=0,
    )
...
def test_adapter_contract_and_stable_identity():
    item = convert(
        ...
        direction="right",
        line_crossing=("gate",),
        evidence_pointer="evidence/1.jpg",
    )
...
def test_unknown_and_untracked_sentinels():
    item = convert(
        object_id=UNTRACKED_OBJECT_ID, confidence=-0.10000000149, class_id=999
    )
...
def test_clips_to_frame():
    assert convert(rect_params=NS(left=-10, top=0, width=1100, height=100)).bbox == (
        0,
        0,
        1,
        0.1,
...
def test_live_frame_traversal_with_sdk_double(monkeypatch):
    obj = NS(
        rect_params=NS(left=0, top=0, width=10, height=10),
        confidence=0.8,
        object_id=1,
        class_id=0,
    )
    ...
    result = adapter.convert_frame(
        frame, timestamp=datetime.now(timezone.utc), frame_width=100, frame_height=100
    )
    assert len(result) == 2
    assert result[0].observation_id != result[1].observation_id
    frame.obj_meta_list = None
    assert (
        adapter.convert_frame(
            frame,
...
def test_sdk_import_boundary():
    ...
    for file in root.rglob("*.py"):
        ...
        for node in ast.walk(ast.parse(file.read_text())):
            ...
            assert not any(
                any(part in {"pyds", "gi", "deepstream"} for part in name.split("."))
                for name in names
            ), file
...
"""
    result = subprocess.run(
        [sys.executable, "-c", script, "replay", str(path)],
        capture_output=True,
        text=True,
