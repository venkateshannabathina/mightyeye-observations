...
`convert_frame` traverses the SDK linked object list and copies scalar data immediately; SDK objects never escape. It leaves analytics and evidence fields null. If your DeepStream analytics module has zone/direction/line outputs, invoke `convert_object` inside that module with your translated `zone`, `direction`, `line_crossing`, and `evidence_pointer` values. Automatic nvdsanalytics user-meta parsing is not implemented: the pipeline must select its primary zone and translate its configured labels. Unsupported detector classes become `unknown`.
